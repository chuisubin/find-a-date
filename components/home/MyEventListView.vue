<template>
  <div class="mx-auto max-w-screen-md">
    <h2 class="text-xl lg:text-3xl font-bold mb-3 lg:mb-5 w-full">
      我參與的聚會
    </h2>
    ...
    <ul class="">
      ...
      >
        <div
          class="absolute top-0 bottom-0 left-0 w-1.5 h-full"
          :class="{
        ...
        <div class="flex flex-row items-center">
          <div class="w-full">
            <div
              class="flex flex-col justify-center items-start gap-1 lg:gap-2"
            >
              <div class="status-container">
                ...
                >
                  投票中 
                  <span class="">{{ formatDeadlineText(event.deadline_date) }}</span>
                </span>
                <span
                  v-else-if="event.status === 'decided'"
                  class="status bg-status-decided-bg text-status-decided-text"
                >
                  已定案 <span class="">{{ event.decided_date }}</span>
                </span>
                <span
              ...
              <div
                v-if="event.status"
                class="text-sm lg:text-base sub_title_color"
              >
          ...
          <button
            @click="goToEvent(event.public_code)"
            class="ml-4 p-2 rounded-full hover:bg-gray-200 text-gray-500 hover:text-gray-600 transition h-fit"
          >
            <svg
      ...
      <li v-if="myEvents.length === 0" class="text-gray-400 text-center py-8">
        No events found.
      </li>
    </ul>
  </div>
...
const formatDeadlineText = (date) => {
  return calculateDeadlineText(date);
}
</script>
